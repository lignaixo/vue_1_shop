  ...
  <main id="main" class="container">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb small bg-transparent">
        ...
        <li class="breadcrumb-item text-dark font-weight-bold small active" aria-current="page">
          笔记本电脑
        </li>
      </ol>
    </nav>
      ...
      <div class="row pr-3">
        <div id="preview" class="col pr-0">
          <div style="width: 352px" class="card bg-transparent mr-5 position-absolute">
            ...
            <div id="mask" class="position-absolute" :class="{'d-none':hide}" :style="maskMove"></div>
            <div id="super-mask" class="position-absolute" @mouseover="toggle" @mouseout="toggle" @mousemove="move">
            </div>
            <!-- 难点四：大图片反向等比例移动 -->
            <div id="div-lg" class="position-absolute"
              :style="{'background-image': `url(${pics[i].lg})`,'background-position':`-${parseInt(maskMove.left)*2}px -${parseInt(maskMove.top)*2}px`}"
              :class="{'d-none':hide}">
            </div>
            <div class="card-body p-0 text-center">
              ...
              <div class="d-inline-block pt-2 mx-0 m-auto">
                ...
                <ul class="list-unstyled mb-0" :style="{width:62*pics.length+'px','margin-left':-62*times+'px'}"
                  @click="changeI">
                  <!-- pics动态绑定 -->
              ...
              <img src="/img/product_detail/hover-next.png" class="btn float-right btn-light border-0 p-1 pt-4 pb-4"
                :class="{disabled:times>=pics.length-4}" @click="moveLeft">
            </div>
          </div>
        </div>
        <div id="details" class="col pl-0">
          ...
          <div class="alert alert-secondary small" role="alert">
            <div>
              <span>学员售价：</span>
              <h2 class="d-inline text-primary font-weight-bold" v-text="`¥${product.price}`"></h2>
            </div>
            <div>
              <span>服务承诺：</span>
          ...
          <div>
            ...
            <div class="float-left w-75">
              <!-- 动态绑定specs -->
              <router-link class="btn btn-sm btn-outline-secondary" :class="{active:sp.lid==lid}"
                v-for="(sp,i) of specs" :key="i" :to="`/details/${sp.lid}`" v-text="sp.spec"></router-link>
            </div>
        ...
        <ul class="list-unstyled" style="width: 1100px; margin-left: -220px">
          <li class="float-left">
            <div class="card mb-1 p-4 border-0 bg-transparent box-shadow">
              <img class="card-img-top"
                ...
                alt="Thumbnail [100%x225]" src="/img/product_detail/product_detail_1.png" data-holder-rendered="true" />
          ...
          <li class="float-left">
            <div class="card mb-1 p-4 border-0 bg-transparent box-shadow">
              <img class="card-img-top"
                data-src="holder.js/100px225?theme=thumb&amp;bg=55595c&amp;fg=eceeef&amp;text=Thumbnail"
                alt="Thumbnail [100%x225]" src="/img/product_detail/product_detail_2.png" data-holder-rendered="true" />
              <div class="card-body p-0">
                <p class="card-text text-center small">
          ...
          <li class="float-left">
            <div class="card mb-1 p-4 border-0 bg-transparent box-shadow">
              <img class="card-img-top"
                data-src="holder.js/100px225?theme=thumb&amp;bg=55595c&amp;fg=eceeef&amp;text=Thumbnail"
                alt="Thumbnail [100%x225]" src="/img/product_detail/product_detail_3.png" data-holder-rendered="true" />
              <div class="card-body p-0">
                <p class="card-text text-center small">
          ...
          <li class="float-left">
            <div class="card mb-1 p-4 border-0 bg-transparent box-shadow">
              <img class="card-img-top"
                data-src="holder.js/100px225?theme=thumb&amp;bg=55595c&amp;fg=eceeef&amp;text=Thumbnail"
                alt="Thumbnail [100%x225]" src="/img/product_detail/product_detail_4.png" data-holder-rendered="true" />
              <div class="card-body p-0">
                <p class="card-text text-center small">
...
<script>
  export default {
    ...
    methods: {
      //难点三：mask的显示隐藏及随鼠标移动在super-mask上操作
      move(e) {
        let top = e.offsetY - 88;
        let left = e.offsetX - 88;
        top < 0 ? top = 0 : top > 176 ? top = 176 : top;
        left < 0 ? left = 0 : left > 176 ? left = 176 : left;
        this.maskMove.left = left + "px";
        this.maskMove.top = top + "px";
      },
      toggle() {
        this.hide = !this.hide;
      },
      //难点二：父元素绑定事件，点击后子元素根据自定义属性切换显示
      changeI(e) {
